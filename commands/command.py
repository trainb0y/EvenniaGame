...
class CmdStats(default_cmds.MuxCommand):
    """ 
    View a visible player's statistics
    ...
    def func(self):
        ...
        if self.args == []: target = self.caller # Assume they mean themselves
            ...
            try: target = target[0]
            except IndexError: target = False # No targets
        ...
        table = evtable.EvTable("Stat","Value","Stat","Value",table=[[ # Headings

                                    "HP","Armor","Magic"       # Column 1
                                ],[
                                    str(target.db.hp)+"/"+str(target.db.max_hp),  # Column 2
                                    target.db.armor,
                                    f"{target.db.magic}/{target.db.max_magic}",
                                ],[
                                    "Str","Dex","Int"        # Column 3
                                ],[
                                    target.db.strength,target.db.dexterity,target.db.intelligence  # Column 4
                                ]])
